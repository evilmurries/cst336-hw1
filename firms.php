<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Cybersecurity</title>
    <link href="css/styles.css" rel="stylesheet" type="text/css" />
    <link href="https://fonts.googleapis.com/css?family=B612+Mono&display=swap" rel="stylesheet"> 
</head>
<body>
    <header>
        Cybersecurity and Cybercrime
    </header>
    <hr>
    <main>
        <nav>
                <br/>
                <a href="index.php">Home</a><br/><br/>
                <a class="currentPage" href="firms.php">Companies</a><br/><br/>
                <a href="types.php">Types</a><br/><br/>
                <a href="practices.php">Best Practices</a><br/><br/>
        </nav>
        <div id="firms">
            <h1>
                <strong>Cybersecurity Companies</strong>
            </h1>
            <img src="img/professionals.jpg" alt="Picture of a group of cybersecurity professionals">
            <br/>
            <p>
                There are numerous firms that specialize in the prevention and remedy of cybercrime.
                Cybersecurity is a rapidly growing industry with numerous avenues for specialization.
                According to <a class="paralink" href="https://cybersecurityventures.com/cybersecurity-market-report/">cybersecurityventures.com</a>, 
                cybersecurity will be a trillion dollar industry by the year 2021. Every business is in need
                of some level of protection, and there are numerous firms out there willing to provide services.
                Here are just a few:
            </p>
            <ul id="companies">
                <li><span class="companies">Cisco Systems</span></li>
                <li><span class="companies">CyberArk</span></li>
                <li><span class="companies">Amazon Web Services</span></li>
                <li><span class="companies">Check Point Software</span></li>
                <li><span class="companies">Palo Alto Networks</span></li>
                <li><span class="companies">Amazon Web Services</span></li>
            </ul>
            <p id="reference">
                Company list provided by: <a href="https://investingnews.com/daily/tech-investing/cybersecurity-investing/top-cyber-security-companies/">investingnews.com</a>
            </p>
        </div>
    </main>
    <footer>
        <hr>
        <img src="img/csumb_logo_blue.png" alt="CSUMB Logo Image"><br/>
        CST336 Internet Programming. 2019&copy; Piwarski <br />
        <strong>Disclaimer:</strong> The information in this webpage is used
        for academic purposes only.
    </footer>
</body>
</html>